...
import com.wanted.cleanarchitecture.catalog.domain.model.ContentModule;
...
@Repository
@Transactional
// 어댑터에서 레파지토리를 implements함. JPA 사용 위해?
public class CourseRepositoryAdapter implements CourseRepository {
    ...
    // CourseRepository 에서 써놓은 save가 여기서 구현됨
    @Override
    public Course save(Course course) {
        ...
        // 엔티티로 바꿔주고 있어서 섹션 추가해도 레파지토리에 Course save가 하나밖에 없음. 이 구조의 큰 특징.
        // 강의 생성 메커니즘.
        CourseJpaEntity entity = course.getId() == null
                // 널값이면 다음줄 아니면 다다음줄 실행. 새로운 코스엔티티 생성, 널값 아니면 수정이므로
        ...
        // 이거는 수정일 때 메커니즘. (강의 생성이어도 그냥 같은값을 넣어줌.)
        ...
        CourseJpaEntity saved = repository.save(entity); // 엔티티에 바뀐 값으로 저장됨
        return toDomain(saved);
        // 리턴해주고있는 값: Course 도메인. 도메인이 읽을 수 있는 형태로 바꿔주는 것.
    }
