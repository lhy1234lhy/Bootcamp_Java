...
public class CourseCommandService implements CourseCommandUseCase {
    ...
    // 처음 만들면 에러뜸. alt+enter해서 handle()들 (USeCase에서 가져온것) 추가해야됨
    ...
    public void handle(AddSectionCommand command) {
        Course course = getCourse(command.courseId()); // id에 해당하는 코스 갖고와서
        course.addSection(command.title(), command.sectionOrder()); // 안에 섹션을 추가하는거임.
        courseRepository.save(course);
    }

    @Override
    public void handle(AddModuleCommand command) {
        Course course = getCourse(command.courseId()); // 강의가 있는지 확인하면서 가져오기
        course.addModule(command.sectionOrder(), command.title(), command.contentType(), command.moduleOrder());
        courseRepository.save(course); // 비로소 db에 저장이 됨
    }
    ...
    private Course getCourse(Long courseId) {
        return courseRepository.findById(courseId) // 해당하는 강의 id 불러옴
