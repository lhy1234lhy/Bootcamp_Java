...
public class Course {
    ...
    private final List<CourseSection> sections; // 리스트로 미리 지정해줌
    ...
    public void addSection(String title, int sectionOrder) {
        ensureDraft();
        boolean duplicatedOrder = sections.stream() // 똑같은 번호가 이미 있는지..
                .anyMatch(section -> section.getSectionOrder() == sectionOrder);
                // 같은 섹션 번호 있으면 TRUE 뜨고 중복처리.
        if (duplicatedOrder) {
        ...
        // 중복 없으면 섹션 추가. SECTION이 리스트라서 add 쓸 수 있는거임
    }

    public void addModule(int sectionOrder, String title, ContentType contentType, int moduleOrder) {
        ensureDraft();
        CourseSection section = sections.stream() // 리스트 형태인 sections 에서 하나의 값만 빼오는거
    ...
    public void publish(Instant occurredAt) {
        ensureDraft();
        validatePublishable(); // 공개 상태로 바꾸기
        this.status = CourseStatus.PUBLISHED;
        this.domainEvents.add(new CoursePublishedEvent(this.id, occurredAt)); // 도메인 이벤트 발생
    }

    // 상태 검증. 도메인에서 이런거도 다 한다. 어느 상태에 뭐 할건지
    ...
    public List<DomainEvent> pullDomainEvents() {
        ...
        domainEvents.clear(); // 클리어를 왜 하냐? -> 메모리상 도메인 이벤트가 남아있으면 또 쓸 때 중복 발생할 수 있다.
        return events;
    }
