...
public class CourseController {
    ...
    // void이므로 CourseCommandUseCase 에서 void 타입으로 맞춰서 작성함
    public ResponseEntity<ApiResponse<Void>> addSection(
        ...
        // 커맨드에서 조립해서 주는거임.
        // 여기서 프론트앤드 애들한테 보내줘야되는 값을 설정해주는 것임.
    ...
    public ResponseEntity<ApiResponse<Void>> addModule(
            ...
            @PathVariable Long courseId, // /{courseId}/sections/{sectionOrder}/modules 여기서 가져옴.
            @Parameter(description = "모듈을 추가할 섹션 순서", example = "1")
            @PathVariable int sectionOrder,
            @Valid @RequestBody AddModuleRequest request
    ) {
        // 이거 만들기 전에 UseCase를 만들고, AddModuleCommand(요청받은 값들) 만들고 그 다음에 이거 만듦
        // request를 분리해서 서비스에 보낼거를 커맨드로 만들어 보내줌. (DTO는 세가지다. (커맨드,))
