...
public class JwtTokenProvider {
    ...
    public String createAccessToken(Authentication authentication) {
        ...
        /* hi.
            jwt.builder 로 만들어지는 정보가
            실제 Front 에서 활용할 수 있는 로그인 유저 관련 정보다
            현재는 userID 와 권한이 들어있어서 Front에서는
            이 2개의 값을 바탕으로 활용할 수 있게 된다.
        */
        return Jwts.builder()
                // 권한, PK, 유니크한 값 정도만 들어가면 된다. (이름, 닉네임은 굳이?)
                .subject(username)
                .subject("추가적인 값") //
                .claim("roles", authorities) // 권한은 반드시 넣는게 좋다
                .issuedAt(new Date())
