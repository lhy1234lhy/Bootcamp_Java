...
@Configuration
/* Spring Secutrity 설정을 활성화 하기 위해 반드시 필요한 어노테이션 */
@EnableWebSecurity
public class SecurityConfig {
    ...
    @Bean
    // 정적 리소스에 대한 요청을 제외하겠다는 설정이다. (custom 객체)
    // (static 파일 하위에 css, js, img, video 깉은 요청)
    public WebSecurityCustomizer webSecurityCustomizer() {
        // requestMatchers는 요청 url이다. ex) "/css/**", "/js/**" 등등
    ...
    *   로그인 성공 시 어떤 동작을 할 지, 실패 시 어떤 동작을 할 지 Custom Handler도 등록할 수 있다.
    *  */
    @Bean
    public SecurityFilterChain configure(HttpSecurity http,
        ...
        /* Http req, res 관련 Security 설정을 하는 공간 */
        // 접두사를 잘 정리해둬야 코드가 너무 복잡해지지 않음
        http.authorizeHttpRequests(auth -> { // 서버의 리소스에 접근 가능한 권한을 설정함
                    // 인증한 사용자가 아니더라도 접근을 가능할 수 있게 permitAll()
                    ...
                    rememberMe.tokenValiditySeconds(86400); // 토큰 유효 기간
                    ...
                    // 사용자 1명 당 1개의 최대 세션을 통해 중복 로그인 방지할 수 있다.
                    session.maximumSessions(1).sessionRegistry(sessionRegistry); // session의 허용 개수를 제한
                    session.invalidSessionUrl("/"); // 세션 만료 시 이동할 페이지 (로그인 바로 다시할 수 있게 하려면 /login)
                })
                /**
                 ...
                 * 3. Postman/REST 클라이언트 테스트 용이: API 테스트 시 CSRF 토큰 처리 불필x요
                 *
                 * 프로덕션 환경에서는:
