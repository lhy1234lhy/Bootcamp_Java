...
public class UserController {
    ...
    public ModelAndView signup(@ModelAttribute SignupDTO signupDTO, ModelAndView mv){
        // signupDTO로 request, ModelAndView로 response 받기 (요청, 응답)

        // dml -> 회원가입 완료된다? 데이터 1개 행 추가되면 server에 영향을
        // 받은 행의 수 만큼 정수값을 반환해준다.
        Integer result = memberService.regist(signupDTO);

        String message = null;

        if(result == null ){ // null -> 중복 ID
            message ="중복회원이 존재합니다."; // null -> 중복 ID
        }else if(result == 0) { // 0 -> 서버 오류
            message="서버에서 오류가 발생하였습니다.";
            // 메세지 출력 후 다시 회원가입 유도
            mv.setViewName("user/signup");
        }else if(result >= 1){ // userCode -> 정상 동작
