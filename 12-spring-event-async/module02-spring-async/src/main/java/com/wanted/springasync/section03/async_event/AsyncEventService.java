...
//        asyncNotificationService.sendCompletionEmail(enrollment);
        // 이벤트 쓴다고 비동기 처리 되는게 아님.
        // 이거 실행되면 리스너 실행되고 밑에 로그부터 다시 실행되는거임
        ...
        return LectureResponse.completed(
                ...
                "핵심 트랜젝션만 처리하고 수료 이메일 발송은 이벤트 라스너에 위임.",
                start
        );
    }
    ...
    public CompletionSummaryResponse requestCompletionSummary(Long enrollmentId) {
        ...
        /* hi.
        *   thenAccept() 의 흐름
        *   1. createSummaryAstnc  메서드 호출 시 CompletableFuture 타입의 값을 즉시 받는다.
        *   2. thenAccpt(매개변수 -> 실행 식) 으로 나중에 성공하면 실행 될 콛를 작성한다.
        *   3. 현재 메인 요청 스레드는 콜백 실행을 기다리지 않고 메인 흐름을 계속 진행한다.
        *   4. 비동기 작업이 모두 완료되면 summary 값이 콜백 함수에 전달되고, 그때 로그가 출력된다.
        *  */
        // 여기서는 반환값이 있어서
        completionSummaryService.createSummaryAsync(enrollmentId).thenAccept(
                ...
                // 이런식으로 활용할 수 있게 된다
                summary -> log.info(
                        "[section03] CompletableFuture 콜백 실행.summary ={}, thread = {}",
                        summary, Thread.currentThread().getName()
                )
        );

        log.info("[section03] CompletableFuture 수강 완료 요청 종료!! 작업을 처리 중인 Thread = {}",
                Thread.currentThread().getName());

        return CompletionSummaryResponse.accepted(
                "메인 흐름 완료. 비동기 수강 요약 생성은 백그라운드에서 계속 진행중!",
                start
        );
    }
    ...
    public CompletionSummaryResponse waitCompletionSummary(Long enrollmentId) {
        ...
        log.info("[section03] CompletableFuture 수강 완료 요청 대기 시작!!. 작업을 처리중인 Thread = {}", Thread.currentThread().getName());
        ...
        /* hi.
        *   join() 은 현재 메인 흐름의 요청 스레드를 멈춰세우고, Future 결과가 채워질 때까지 기다린다.
        ...
        *   메인흐름으로 넘어가는 방식으로 사용하게 된다.
        *  */
        String summary = future.join();

        log.info("[section03] CompletableFuture 수강 완료 요청 대기 종료!!. 작업을 처리중인 Thread = {}", Thread.currentThread().getName());
