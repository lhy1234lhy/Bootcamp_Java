...
public class CertificateService {
    ...
    *   - propagation : 전파의 의미를 가지고 있고 default value는
    *   REQUIRED (상위 틀렌젝션이 있다면 같이 묶인다.)
    *   REQUIRED_NEW (상위 트랜젝션과 분리된 독립된 트랜젝션 경계를 만든다.)
    *   - rollbackFor : 예외는 크게 2가지가 있다. 언체크 예외, 체크 예외
    *   체크예외 시에도 rollback 하기 위해서 작성해야 한다.
    *   ex) rollbackFor = Exception.class
    *   - timeout = 3
    *   너무 오래 걸리는 작업을 제한할 때 사용한다.
    *   (성능최적화를 한 이유를 생각하며 해라)
    * */

    // AFTER_COMMIT이 있어서 이거 필요없다
