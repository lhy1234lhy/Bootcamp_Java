...
-- SQL 기초 강의를 수강하는 학생들의 평균 점수보다 높은 점수를 
-- 받은 학생의 이름은 무엇인가?
...
-- '홍길동' 사용자와 같은 해에 가입한 모든 사용자의 이름과 가입일 조회
-- 1. 
select 
	name, 
    created_at
...
-- 'Python' 태그가 붙은 과정을 수강 신청한 모든 학생의 user_id와 name, course_id 조회하라.
-- enrollments, tag, courses join 이 연산됨.
-- 1.alter
select
	u.user_id,
    u.name,
...
where e.course_id in (
	-- 여기서 도출해야 하는 값은 course_id가 Python 태그를 포함하고 있는 것을 판단.
    select
		c.course_id
	from courses c
