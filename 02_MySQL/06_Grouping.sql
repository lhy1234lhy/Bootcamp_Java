...
-- user_profiles 테이블에서 bio(자기소개) 를 작성한 사용자는 몇 명인지 소환
select count(bio) as '자소서 작성자 수'
from user_profiles;

-- courses  테이블에 과정을 등록한 교육자가 총 몇명인지 중복없이 계산 후 조회 
select count(distinct author_id) as '과정 등록한 교육자 수'
from courses;
...
-- 하지만 sleect 절에 집계 함수와 함께 Group by 절 없이 일반 컬럼(비집계)을
...
-- 각 그룹에 대해 집계함수를 적용하여 소계를 계산하는 기능이다.
...
-- courses 테이블에서 각 교육자가 몇 개의 과정을 개설했는지 조회하라.	
 select
	author_id,
    count(*) as '개설 강좌 수'
from courses
...
-- 가장 큰 차이점은 where는 데이터를 그룹화 하기 전 필터링 한다.
-- having 절은 그룹화 후 집계함수를 계산 한 후에 필터링 한다.
...
-- 해당 학생 수를 조회한다 (where, having 같이 쓴다)
select course_id, count(*) as '등록 학생 수'
from enrollments
...
-- 문제 상황 제기
-- 월별로 데이터를 그룹화 해야 하는데
-- 실제 created_at 컬럼의 데이터는 일과 시간도 작성 되어 있다.
-- 따라서 우리는 created_at 컬럼의 데이터를 format 후 그룹화 할 것이다.
-- 내장 함수 맛보기 DATE_FROMAT(날짜, 형식) 날짜 데이터를 원하는 문자열 형식으로 바꿔주는 함수

select 
	DATE_FROMAT(created_at, '%Y-%m') as '가입 월',
    count(*) as '월별 가입자 수'
from users
