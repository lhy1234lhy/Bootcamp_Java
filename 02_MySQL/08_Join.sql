...
-- 테이블 정보는 각각 테이블에 저장이 되어있다.
-- 테이블을 나누는 것은 정규화 하고 하며, 정규화에 의해
-- 비슷한 종류의 데이터끼리 테이블을 구성하게 된다.

-- join 은 이렇게 분리된 테이블들 중 공통으로 포함하는 값(주로 PK-FK)
-- 을 기준으로 다시 합쳐서 의미있는 결과 집합을 만들어낸다. 
...
-- courses 테이블, users 테이블을 조합하여 과정 제목과, 그 과정을 만든 
-- 교육자 이름을 조회하라
select
	c.title as '과정 제목',
    u.name as '교유자 명'
...
-- 테이블 3개 조인하기
-- enrollments 테이블 기준으로 users, courses를 연결하여
-- 어떤 학생이 어떤 과정에 등록했는 지 학생 이름과 과정 제목과 등록일을 조회하라.
select 
	name as '학셍 이름', 
    title as '과정 제목',
    enrolled_at as '등록일'
from enrollments as e
inner join users as u on e.user_id = e.user_id
inner join courses as c on e.course_id = c.course_id;

select * from enrollments;

-- left join
-- inner join은 양쪽에 모두 데이터가 있는 경우만 보여준다.(교집합)
-- 하지만 요구사항이 만약
...
-- 왼쪽 테이블의 경우 매칭되는 데이터가 없으면, null로 표기된다.

-- user 테이블 기준, user_auth_providers 를 right join 하여
...
-- 하지만 대댓글의 경우 부모 댓글에 대한 정보를 가져야 어던 댓글의 대댓글인 지를 확인할 수 있다.
