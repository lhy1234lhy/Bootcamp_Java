...
create table courses_temp as select * from courses; -- courses 테이블 복사
...
-- drop : ddl 데이터를 정의어. 테이블의 데이터와 구조 자체를 영구적 삭제
...
-- 해당 테이블 데이터 초기화
truncate table courses_temp; -- 제약조건이랑 컬럼같은 제목들은 남아있다. 데이터만 사라짐 (복구가 힘듦(롤백 거의 불가능))
