...
/*
    ...
    - 트랜잭션이 없다면, 홍길동은 3000원을 잃고 김철수는 3000원을 받지 못한다.
    
    *데이터의 일관성이 지켜지지 않았다고 표현을 한다.
    
    트랜잭션은 두 개의 update 구문을 하나의 논리적인 작업 단위로 묶어,
    두 작업이 모두 성공하거나, 또는 하나라도 실패를 하게 되면 원상복구 하도록
    보장하는 기능이다.
    
    이 트랜잭션 기능을 통해 데이터베이스는 어떤 상항이라도
    데이터의 무결성과 일관성을 지킬 수 있다.
*/
