...
-- SELECT 구문은 관리 혹은 저장된 데이터를 어떤 식으로 활용할 지를 담당하는 명령어이다.
-- SELECT 구문의 핵심 2가지 
-- 1. 어떤 열을 가져올 것인가?(정보의 종류)
-- 2. 어떤 행을 가져올 것인가?(정보의 범위)

-- * : 에스터리스크 라고 불리우며, 모든 열을 의미하는 와일드카드이다.
select 
	*
from users;
...
-- where 절은 SQL 심장.
-- 수많은 데이터 속에서 원하는 조건의 데이터만 정호가하게 필터링하는
-- where 를 얼마나 잘 사용하느냐에 따라 SQL의 성능과 정확성이 결정된다.

-- users 테이블에서 user의 status가 active 가 
-- 아닌 모든 사용자의 user_id와 status 조회
/*
	비교연산자
    = : 같다
    != : 같지 않다
...
-- Null 값 확인 구문
-- is null : g=해당 컬럼의 값이 비어있는 레코드 조회
-- is not null : 해당 컬럼의 값이 비어있지 않는 레코드 조회

-- user 테이블에서 user_id, 이름, 마지막 로그인 일시 조회를 할건데
-- 마지막 로그인 일시가 null인 사용자만 조회
...
-- 2024년 3월 이후에 가입했고,  Status 가 Active 인 사용자의 이름과 이메일 조회
select name, email
from users
...
-- limit 는 '상위 5개만 보기', '1페이지에 10개씩 보기' 등
-- 웹 어플리케이션 페이징 기능을 구현하는데 필수 핵심 문법이다.
...
-- users 테이블에서 아이디, 이름, 가입일을 조회할건데 가장 나중에 가입한 순서대로 나열 (오름차순)
select user_id, name, created_at
from users
...
-- 5. 두 번쨰 페이지에 해당하는 5명(6~~10번째)의 user_id, name, email, created_at을 조회하라.
...
from users
where (created_at between '2024-04-01 00:00:00' and '2024-05-31 23:59:59')
and status = 'active' 
