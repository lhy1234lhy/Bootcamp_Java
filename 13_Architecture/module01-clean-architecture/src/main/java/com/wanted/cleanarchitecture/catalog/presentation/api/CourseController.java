...
import com.wanted.cleanarchitecture.catalog.application.usecase.SectionCreateUseCase;
import com.wanted.cleanarchitecture.catalog.presentation.api.request.CreateCourseRequest;
import com.wanted.cleanarchitecture.catalog.presentation.api.request.CreateSectionRequest;
import com.wanted.cleanarchitecture.catalog.presentation.api.response.CreateCourseResponse;
import com.wanted.cleanarchitecture.catalog.presentation.api.response.CreateSectionResponse;
...
//                        "COURSE_CREATED" // 이미 공통으로 쓰는거 만들어놔서 하드코딩 안 해도 갖다 쓰면 됨
    ...
    public ResponseEntity<ApiResponse<Void>> addSection(
        ...
        courseCommandUseCase.handle(
                new CreateSectionCommand(courseId, request.title(), request.sectionOrder())
        );
