...
// 사용자가 인터넷 브라우저에 특정 주소(URL)를 입력하고 들어왔을 때,
// 어떤 자바 클래스나 메서드가 이 요청을 맞이할지 연결함
public class RequestController {

    /* hi.
     *   view 페이지를 보여주는 방식은 여러가지가 있다.
     *   1. String 타입의 반환값으로 view 이름을 작성
     *   2. 메소드 타입을 void 로 하게 되면
     *   - 요청 url이 view의 이름이 된다.
     *   public String test(){
     *       return "requst/regist";
     *   }
     *   public void regist(){}
     *  */

    // 빈 폼(HTML 화면)을 보여주는 역할
    @GetMapping("regist")
    public void regist() {}
    /* 위 코드와 동일함. @GetMapping("regist") public String test() { return "request/regist"; } */

    // 사용자가 보낸 데이터(name, price 등)를 받아서 계산하거나 데이터베이스에 저장한 뒤,
    // '성공했습니다!'라는 결과 화면을 보여줌
    @PostMapping("regist") // 서블릿의 doPost같은 역할
