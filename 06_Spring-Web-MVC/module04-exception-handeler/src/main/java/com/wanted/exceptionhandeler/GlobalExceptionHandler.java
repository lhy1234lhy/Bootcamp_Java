...
public class GlobalExceptionHandler {
    ...
    *  */
    ...
    public String globalNull(Model model, NullPointerException e){
        System.out.println("전역 레벨의 예외처리!");
        model.addAttribute("key", e.getMessage());
        return "error/nullPointer";
    }

    @ExceptionHandler(MemberNotFoundException.class)
    public String globalMemberExceprion(Model model, MemberNotFoundException e){
    ...
    /* hi.
    *   예외처리 클래스이 부모인 Exception 클래스를 Handling
    *   예외처리 클래스는 모두 Exception을 상속받아 만들어져있다.
    *   또한 우리가 개발 시 발생할 수 있는 모든 예외를 핸들링 할 수 없기 때문에
    *   대표적인 예외 처리들만 핸들링, 사용자 정의의 예외처리들만 핸들링을 해두고
    *   그 외 예외는 Excption으로 default 설정을 해둔다.
    *  */
    @ExceptionHandler(Exception.class)
    public String defaultException(Model model, Exception e){
        model.addAttribute("class", e.getClass());
        model.addAttribute("message", e.getClass());
