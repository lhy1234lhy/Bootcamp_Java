...
public class FileController {
    ...
    *  */
    ...
    // 이 경우에는 이미지 파일 이름이 동일하기 떄문에 평훈이 이미지 파일 이름을 수정하면
    ...
    public String singleUpload(@RequestParam MultipartFile singleFile, @RequestParam String description, Model model) throws IOException {
        ...
        // classpath: src/main/resources를 의미(줄임말?) 저장할 공간을 만들어줌
        ...
        // 파일의 확장자 추출 ex) .png  (. 기준으로 뒤에 있는 글자(확장자) 추출)
        ...
        try{
            singleFile.transferTo(new File(filePath + "/" + savedName)); // 저장
            model.addAttribute("message", "파일 업로드 성공!"); // 업로드 성공
            // 파일 저장 경로 view로 전달
            model.addAttribute("img", "/static/img/single/" + savedName); // 모델에 담아서 던짐
        } catch(Exception e){
    ...
    public String multiFileUpload(@RequestParam("multiFile") List<MultipartFile> multipartFile, String description, Model model) throws IOException {
        ...
        if(!resource.exists()){
            ...
            filePath = file.getAbsolutePath(); // 업로드한 파일의 경로
        } else{
            filePath = resourceLoader.getResource("classpath:static/img/multi")
                    .getFile().getAbsolutePath();
            // 리소스가 존재하지 않으면 새로운 파일을 만들게 함
        }

        // 여러 개의 파일을 처리하기 위한 DTO 파일 생성
