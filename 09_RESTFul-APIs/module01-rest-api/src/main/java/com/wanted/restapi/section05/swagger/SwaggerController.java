...
public class SwaggerController {
    ...
    @ApiResponses(value = {
            ...
            // hidden : 스웨거 문서에 상태 코드(500, 404 등)와 설명만 띄우고 데이터 구조는 화면에 그리지 말고 감춤
            @ApiResponse(responseCode = "500", description = "서버 내부 오류", content = @Content(schema = @Schema(hidden = true)))
    })
    ...
    // 숙제 : Post, Put에 swagger 어노테이션 달기, Delete도 해보기

    @Operation(summary = "신규 유저 등록", description = "신규 유저 등록한다.")
    @ApiResponses(value = {
...
//                .body(newUser);   // postman 에서 데이터 코드 밑에 나옴.(newUser (=UserDTO)보임)
    ...
    @Operation(summary = "유저 정보 수정", description = "사용자 번호를 통해 유저 정보 수정한다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "조회 성공", content = @Content(schema = @Schema(implementation = ResponseMessage.class))),
            @ApiResponse(responseCode = "404", description = " 사용자를 찾을 수 없음", content = @Content(schema = @Schema(hidden = true))),
            @ApiResponse(responseCode = "400", description = " 잘못된 입력 요청", content = @Content(schema = @Schema(hidden = true))),
            @ApiResponse(responseCode = "500", description = "서버 내부 오류", content = @Content(schema = @Schema(hidden = true)))
    })
    @Parameter(name = "userNo", description = "조회할 사용자 번호", required = true)
    // 유저 정보 수정
    ...
    @Operation(summary = "유저 삭제", description = "사용자 번호를 통해 유저 정보 삭제한다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "삭제 성공", content = @Content(schema = @Schema(implementation = ResponseMessage.class))),
            @ApiResponse(responseCode = "404", description = " 사용자를 찾을 수 없음", content = @Content(schema = @Schema(hidden = true))),
            @ApiResponse(responseCode = "400", description = " 잘못된 입력 요청", content = @Content(schema = @Schema(hidden = true))),
            @ApiResponse(responseCode = "500", description = "서버 내부 오류", content = @Content(schema = @Schema(hidden = true)))
    })
    ...
    public ResponseEntity<?> deleteUser(@PathVariable("userNo") int userNo) {
        ...
        // 1-1. 실제 데이터가 있는지 먼저 확인 (안전한 삭제를 위해)
        UserDTO foundUser = users.stream()
                .filter(user -> user.getNo() == userNo)
                .findFirst()
                .orElseThrow(() -> new NoSuchElementException("해당 번호의 사용자가 없습니다."));

        // 1-2. 인덱스가 아닌 '객체' 자체를 넘겨서 삭제 (List.remove(Object))
        users.remove(foundUser);

        // 2. 안전하게 리스트에서 데이터 삭제하기 (번호가 일치하면 삭제)
        //users.removeIf(user -> user.getNo() == userNo);


        // 삭제가 정상적으로 완료되었다면 204 No Content (바디 없는 성공 응답)를 반환합니다.
        return ResponseEntity
                .noContent()
                .build();
    }
